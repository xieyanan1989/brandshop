            ...
            },
            {
                label: '操作', width: 160, align: 'center', sortable: false, formatter: function (value, col, row) {
                    return '<button class="btn btn-outline btn-info" onclick="vm.lookDetail(' + row.id + ')"><i class="fa fa-info-circle"></i>&nbsp;详情</button>' +
                        '<button class="btn btn-outline btn-primary" style="margin-top: 10px;" onclick="vm.printDetail(' + row.id + ','+row.orderId+')"><i class="fa fa-print"></i>&nbsp;打印</button>';
                }
            }
        ]
    });
...
let vm = new Vue({
	...
	methods: {
        ...
        sendGoods: function (event) {
            ...
            vm.title = "发货";
            Ajax.request({
                url: "../ordergoods/info/" + id,
                async: true,
                successCallback: function (r) {
                    vm.orderGood = r.orderGood;
                }
            });
        },
		...
		saveOrUpdate: function (event) {
			 Ajax.request({
	                type: "POST",
	                url: "../ordergoods/sendGoods",
	                contentType: "application/json",
	                params: JSON.stringify(vm.orderGood),
	                successCallback: function (r) {
	                    vm.reload();
	                }
	            });
        ...
        printDetail: function (rowId,orderId) {
            openWindow({
                type: 2,
                title: '<i class="fa fa-print"></i>打印票据',
                content: '../brandshop/orderPrint.html?orderId=' + orderId + '&id='+rowId
            })
        },
        created: function () {
            let vue = this;
